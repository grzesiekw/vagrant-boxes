...
action :install do
  ...
  artifact_file = ::File.join new_resource.dest, "#{new_resource.artifact_id}.#{new_resource.packaging}"
  artifact_file_with_version = "#{new_resource.artifact_id}-#{new_resource.version}.#{new_resource.packaging}"
  ...
  force_update =  new_resource.update
  plugin_version = '2.4'
  ...
  unless ::File.exists?("#{artifact_file}") and !force_update then
    print "Updating: (#{artifact_file_with_version})" + artifact_file
